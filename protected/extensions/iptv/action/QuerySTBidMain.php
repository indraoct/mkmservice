...
class QuerySTBidMain{
    ...
    public function __construct($UserID,$Password) {
        ...
        Yii::import('application.extensions.iptv.abstract.QuerySTBid.*');
    ...
    public function QuerySTBidACtion(){
        ...
        try{
            ...
            $this->_client->setOperationTimeout(360);
             ...
             $request = new reqQuerySTBid();
                    $request->UserID =  $this->_UserID;
                    $request->Password = $this->_Password;

                    $request1 = new QuerySTBid();
                    $request1->reqQuerySTBid = $request;


                    $this->_data = $this->_client->QuerySTBid($request1);
                    
                     $this->__logapi("QuerySTBid",$this->_UserID,$this->_client->__getLastRequest(),$this->_client->__getLastResponse(),$req_date,date("Y-m-d H:i:s"));
                  ...
                  */
                   if($this->_client->__getLastResponse() != null){                 
                        ...
                        $dataXml = $objXml->children("SOAP-ENV", true)->Body->children("BOSS", true)->QuerySTBIDRsp->children();
                        
                        if(!isset($dataXml->Result)){
                            $dataXml->Result = "";
                        }
                        
                        if(!isset($dataXml->STBID)){
                            $dataXml->STBID = "";
                        }
                        ...
                        if(!isset($dataXml->Errordesc)){
                            ...
                            return CJSON::encode(array("Result"=>$dataXml->Result->__toString(),
                                                     "STBID"=>$dataXml->STBID->__toString(),
             ...
             $this->__logapi("QuerySTBid",$this->_UserID,$this->_client->__getLastRequest(),$e->getMessage(),$req_date,date("Y-m-d H:i:s"));
